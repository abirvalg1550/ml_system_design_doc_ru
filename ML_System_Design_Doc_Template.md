...
[Разница между MVP и целевым продуктом](https://github.com/abirvalg1550/ml_system_design_doc_ru/blob/main/Difference_between_MVP_and_target_product.md) 

### 1. Цели и предпосылки 
#### 1.1. Зачем идем в разработку продукта?  

- Бизнес-цель `Product Owner`
  - Есть ли разница в бизнес-цели MVP и целевым продуктом?
  - Если есть - то какая ? 
- Почему станет лучше, чем сейчас, от использования ML `Product Owner` & `Data Scientist`
  - В разбивке на MVP и целевой продукт 
- Что будем считать успехом итерации с точки зрения бизнеса `Product Owner`
  - Важно! прописать четко это для MVP т.к. на это завязано оценка по ВКР!  
  - Прописать для целевого продукта 
...
- Что мы ожидаем от конкретной итерации `Product Owner`.
  - Что мы ожидаем от реализации MVP ?
    
- Описание бизнес-процесса MVP, насколько это возможно - как именно мы будем использовать модель в существующем бизнес-процессе? `Product Owner`
    -  Это нужно по окончанию 2-го семестра. 
- Что считаем успешным MVP? Критерии успеха и возможные пути развития проекта `Product Owner`
...
- Описание планируемого технического долга (что оставляем для дальнейшей продуктивизации) `Data Scientist`
  ...
  Т.е. что нужно будет доделать для получения Целевого продукта. 
